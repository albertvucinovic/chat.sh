...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "replace_lines", "description": "Simple, predictable line-number editing: action=replace|insert|delete. replace: inclusive [start..end]. insert: before|after start (use start=1,before for beginning; start=N+1,after for append). delete: inclusive [start..end]. new_content lines get trailing newlines.", "parameters": {"type": "object", "properties": {"file_path": {"type": "string"}, "start_line": {"type": "integer"}, "end_line": {"type": "integer"}, "new_content": {"type": "string"}, "action": {"type": "string", "enum": ["replace", "insert", "delete"]}, "position": {"type": "string", "enum": ["before", "after"]}}, "required": ["file_path", "start_line", "new_content"]}}},
