...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "replace_lines", "description": "Simple, predictable line-number editing: action=replace|insert|delete. replace: inclusive [start..end] (end optional). insert: before|after start (use start=1,before for beginning; start=N+1,after for append). delete: inclusive [start..end] (end optional). new_content lines get trailing newlines.", "parameters": {"type": "object", "properties": {"file_path": {"type": "string"}, "start_line": {"type": "integer"}, "end_line": {"type": "integer"}, "new_content": {"type": "string"}, "action": {"type": "string", "enum": ["replace", "insert", "delete"]}, "position": {"type": "string", "enum": ["before", "after"]}}, "required": ["file_path", "start_line"]}}},
    {"type": "function", "function": {"name": "spawn_agent", "description": "Spawn a single child agent using current CWD as working dir. Returns {tree_id,parent_id,child_id,dir,session}.", "parameters": {"type": "object", "properties": {"context_text": {"type": "string"}, "label": {"type": "string"}}, "required": ["context_text"]}}},
    {"type": "function", "function": {"name": "spawn_agent_auto", "description": "Spawn a single child agent using current CWD as working dir with auto-approval for tool calls (EG_YES_TOOL_FLAG=1). Returns {tree_id,parent_id,child_id,dir,session}.", "parameters": {"type": "object", "properties": {"context_text": {"type": "string"}, "label": {"type": "string"}}, "required": ["context_text"]}}},
    {"type": "function", "function": {"name": "wait_agents", "description": "Wait for specific child agent IDs (e.g., label-001) to finish. Use IDs exactly as shown by /tree. Pass [] to wait for all. Optional timeout_sec. Set any_mode=true to return when any completes.", "parameters": {"type": "object", "properties": {"which": {"type": "array", "items": {"type": "string"}}, "timeout_sec": {"type": "integer"}, "any_mode": {"type": "boolean"}}, "required": ["which"]}}},
    {"type": "function", "function": {"name": "write_result", "description": "Write result.json and mark done for the current agent directory.", "parameters": {"type": "object", "properties": {"agent_dir": {"type": "string"}, "return_value": {"type": "string"}, "summary": {"type": "string"}}, "required": ["agent_dir", "return_value"]}}},
    {"type": "function", "function": {"name": "list_agents", "description": "List all children of the current tree, grouped by parent, with status.", "parameters": {"type": "object", "properties": {"tree_id": {"type": "string"}}}}}
]
