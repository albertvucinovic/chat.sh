...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "replace_lines", "description": "Flexible line editing. Supports two indexing modes: indexing='line' (recommended) uses 1-based line numbers; operation='replace' replaces inclusive [start..end]; operation='insert' inserts before/after the given line (position='before'|'after'); special cases: (0,0) inserts at beginning; (N+1,N+1) inserts at end. indexing='boundary' (legacy/back-compat) uses boundaries 0..N between lines; start==end inserts at that boundary; start<end replaces lines [start+1..end]. Rules: required: file_path,start_line,end_line,new_content. Optional: indexing ('line'|'boundary'), operation ('auto'|'insert'|'replace'), position ('before'|'after' when inserting in line mode). With operation='auto': start==end->insert; start<end->replace. File creation allowed only for insert at beginning on missing files. new_content is split into lines; each gets a trailing newline. Examples: replace 4..8 -> {..., start_line:4, end_line:8, indexing:'line', operation:'replace'}; insert after 4 -> {..., start_line:4, end_line:4, indexing:'line', operation:'insert', position:'after'}; insert before 1 -> {..., start_line:1, end_line:1, indexing:'line', operation:'insert', position:'before'}; append -> {..., start_line:N+1, end_line:N+1, indexing:'line', operation:'insert'}; legacy: replace first line -> {..., start_line:0, end_line:1, indexing:'boundary'}; insert at beginning -> {..., start_line:0, end_line:0, indexing:'boundary'}. ",
        "parameters": {"type": "object", "properties": {"file_path": {"type": "string"}, "start_line": {"type": "integer"}, "end_line": {"type": "integer"}, "new_content": {"type": "string"}, "indexing": {"type": "string", "enum": ["boundary", "line"]}, "operation": {"type": "string", "enum": ["auto", "insert", "replace"]}, "position": {"type": "string", "enum": ["before", "after"]}}, "required": ["file_path", "start_line", "end_line", "new_content"]}}},
